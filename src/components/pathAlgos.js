...
export function Dijkstra(graph, hashmap, prevmap, start, end, refArr) {
  ...
  while (queue.length > 0) {
    ...
    if (c.x === end.x && c.y === end.y) return [c, count];

    if (c.x + 1 < 50 && !hashmap[`${c.x + 1}-${c.y}`] && !graph[c.y][c.x + 1].iswall && graph[c.y][c.x + 1].weight === 1) {
      queue.unshift({ x: c.x + 1, y: c.y });
      prevmap[`${c.x + 1}-${c.y}`] = { ...c, weight: count };
      hashmap[`${c.x + 1}-${c.y}`] = true;
    }

    if (c.x - 1 >= 0 && !hashmap[`${c.x - 1}-${c.y}`] && !graph[c.y][c.x - 1].iswall && graph[c.y][c.x - 1].weight === 1) {
      queue.unshift({ x: c.x - 1, y: c.y });
      prevmap[`${c.x - 1}-${c.y}`] = { ...c, weight: count };
      hashmap[`${c.x - 1}-${c.y}`] = true;
    }

    if (c.y + 1 < 26 && !hashmap[`${c.x}-${c.y + 1}`] && !graph[c.y + 1][c.x].iswall && graph[c.y + 1][c.x].weight === 1) {
      queue.unshift({ x: c.x, y: c.y + 1 });
      prevmap[`${c.x}-${c.y + 1}`] = { ...c, weight: count };
      hashmap[`${c.x}-${c.y + 1}`] = true;
    }

    if (c.y - 1 >= 0 && !hashmap[`${c.x}-${c.y - 1}`] && !graph[c.y - 1][c.x].iswall && graph[c.y - 1][c.x].weight === 1) {
      queue.unshift({ x: c.x, y: c.y - 1 });
      prevmap[`${c.x}-${c.y - 1}`] = { ...c, weight: count };
      hashmap[`${c.x}-${c.y - 1}`] = true;
    }
